...
class Reaction:
  ...
  def check_molecule_status(self):
    ...
    for substrate in substrates:
      for ligand in ligands:
        ...
        if distance_min < self.minimum_substrate_ligand_distance:
  ...
  def calculate_dt(self):
    ...
    distance = (distance_min_overall+0.1) *1e-10 # meters
    lamda = 5
  ...
  def find_locked_location(self, substrate, ligand):
    ...
    bound = count_bound_docks(distances, self.binding_distance)
    delta_G = -7.1
    return_to_old_location = determine_thermodynamics(bound, 2, delta_G)

    if bound < 2:
      delta_G = -7.1
      #delta_G = -5
      return_to_old_location = determine_thermodynamics(bound, self.minimum_binding_docks, delta_G)
    elif bound >= 2:
...
@jit(nopython=True)
def determine_thermodynamics(num_bound, minimum_binding_docks, delta_G):
  return_to_old_location = True
  if num_bound < minimum_binding_docks:
    Poff = np.exp(delta_G)
...
def get_U(substrate_dock_locations, ligand_dock_locations, binding_distance):
  ...
  result = (1/2*klongi*r**2)/(kBT)*bound 
  return result
