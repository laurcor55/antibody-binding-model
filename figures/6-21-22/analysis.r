...
df_6_17_22 <- import_data('../6-17-22/output.json')
df <- bind_rows(df, df_6_17_22)
glimpse(df)
print(df$ligand_rotation_offset_deg)

df_plot <- df %>% filter(n_substrates==9)
ggplot(data=df_plot) +
...
ggsave('ligand_rotation.png', width=12, height=3)

ggplot(data=df_plot) +
  geom_point(aes(x=min_substrate_ligand_distance, y=reaction_center_bind_proportion, color=factor(ligand_rotation_offset_deg))) +
  facet_grid(cols = vars(min_substrate_substrate_distance))
ggsave('center_binding_proportion.png', width=12, height=3)
